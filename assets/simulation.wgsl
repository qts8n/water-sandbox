...
fn integrate(@builtin(global_invocation_id) invocation_id: vec3<u32>) {
    ...
    particles[index].velocity += vec3<f32>(gravity.value / fluid_props.mass * fluid_props.delta_time);
    particles[index].position += vec3<f32>(particles[index].velocity * fluid_props.delta_time);

    // Handle collisions
    let half_size: vec3<f32> = fluid_container.size / 2.;
    let ext_min: vec3<f32> = fluid_container.position - half_size + fluid_props.radius;
    let ext_max: vec3<f32> = fluid_container.position + half_size - fluid_props.radius;
